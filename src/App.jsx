...
function BetForm({ editData, userId, onSave, onClose }) {
  const [form, setForm] = useState(editData || {
    data: new Date().toISOString().slice(0,10), evento:'', esporte:'Futebol',
    mercado:'', selecao:'', casa:'Bet365', tipo:'simples', odd:'', valor:'', status:'pendente', observacao:''
  })
    ...
    <div style={{position:'fixed',inset:0,background:'#000000cc',display:'flex',alignItems:'center',justifyContent:'center',zIndex:999,backdropFilter:'blur(4px)',padding:16}}>
      <div style={{background:'#141928',border:'1px solid #2a3048',borderRadius:18,padding:28,width:'100%',maxWidth:500,maxHeight:'90vh',overflowY:'auto'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:13}}>
          ...
          <F k="status" label="STATUS" opts={['pendente','ganhou','perdeu']} />
          <F k="evento" label="EVENTO / JOGO" full />
          <F k="esporte" label="ESPORTE" opts={ESPORTES} />
          <F k="casa" label="CASA DE APOSTAS" opts={CASAS} />
          <F k="tipo" label="TIPO" opts={['simples','múltipla','ao vivo']} />
          <F k="mercado" label="MERCADO" />
          <F k="selecao" label="SELEÇÃO" />
          <F k="odd" label="ODD" type="number" />
          <F k="valor" label="VALOR (R$)" type="number" />
          <F k="observacao" label="OBSERVAÇÃO" full />
        </div>
...
function Analytics({ bets }) {
  ...
  const bankrollData = useMemo(()=>{
    ...
    let bal=0
    return sorted.map(b=>{ const l=b.status==='ganhou'?b.retorno-b.valor:-b.valor; bal+=l; return {data:b.data.slice(5),lucro:+bal.toFixed(2)} })
  },[bets])
  ...
  const byEsporte = useMemo(()=>{
    ...
    bets.filter(b=>b.status!=='pendente').forEach(b=>{
      ...
      if(b.status==='ganhou'){map[e].ganhou++;map[e].lucro+=b.retorno-b.valor}
      else{map[e].perdeu++;map[e].lucro-=b.valor}
    })
  ...
  const byCasa = useMemo(()=>{
    ...
    bets.filter(b=>b.status!=='pendente').forEach(b=>{
      ...
      map[c].total++
      if(b.status==='ganhou'){map[c].ganhou++;map[c].lucro+=b.retorno-b.valor}
      else map[c].lucro-=b.valor
    })
    ...
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      <div style={{background:'#111724',border:'1px solid #1e2538',borderRadius:16,padding:'20px 24px'}}>
        ...
        {bankrollData.length<2
          ? <div style={{color:'#8892a4',textAlign:'center',padding:40}}>Registre mais apostas finalizadas para ver o gráfico</div>
          : <ResponsiveContainer width="100%" height={220}>
              <LineChart data={bankrollData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1e2538" />
                <XAxis dataKey="data" stroke="#8892a4" fontSize={11} />
                <YAxis stroke="#8892a4" fontSize={11} tickFormatter={v=>`R$${v}`} />
                <Tooltip contentStyle={tt} formatter={v=>[`R$ ${v}`,'Saldo']} />
                <Line type="monotone" dataKey="lucro" stroke="#00e676" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>}
      </div>
          ...
          {byEsporte.length===0 ? <div style={{color:'#8892a4',textAlign:'center',padding:30,fontSize:13}}>Sem dados</div>
            : <ResponsiveContainer width="100%" height={200}>
                <BarChart data={byEsporte} layout="vertical">
                  <CartesianGrid strokeDasharray="3 3" stroke="#1e2538" />
                  <XAxis type="number" stroke="#8892a4" fontSize={11} tickFormatter={v=>`R$${v}`} />
                  <YAxis dataKey="name" type="category" stroke="#8892a4" fontSize={11} width={70} />
                  <Tooltip contentStyle={tt} formatter={v=>[`R$ ${v}`,'Lucro']} />
                  <Bar dataKey="lucro" radius={[0,4,4,0]}>
                    {byEsporte.map((e,i)=><Cell key={i} fill={e.lucro>=0?'#00e676':'#ff1744'} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>}
        </div>
            ...
            : <div style={{display:'flex',flexDirection:'column',gap:10}}>
                {byCasa.map((c,i)=>(
                  <div key={i} style={{display:'flex',alignItems:'center',gap:10}}>
                    ...
                    <div style={{fontSize:11,color:'#8892a4'}}>{c.total} ap.</div>
                    <div style={{fontSize:12,color:'#ffab00',width:40,textAlign:'right'}}>{c.winrate}%</div>
                    <div style={{fontSize:13,fontWeight:700,color:c.lucro>=0?'#00e676':'#ff1744',width:80,textAlign:'right'}}>R$ {c.lucro}</div>
                  </div>
                ))}
              </div>}
        </div>
...
function Bankroll({ bets, userId }) {
  ...
  const stats = useMemo(()=>{
    const won=bets.filter(b=>b.status==='ganhou'), lost=bets.filter(b=>b.status==='perdeu')
    const lucroTotal=won.reduce((s,b)=>s+(b.retorno-b.valor),0)-lost.reduce((s,b)=>s+b.valor,0)
    const saldoAtual=config.bankroll_inicial+lucroTotal
    const stopLossVal=config.bankroll_inicial*(config.stop_loss_percent/100)
    const emRisco=lucroTotal<0&&Math.abs(lucroTotal)>=stopLossVal*0.7
    return {lucroTotal,saldoAtual,stopLossVal,emRisco,pct:((lucroTotal/config.bankroll_inicial)*100).toFixed(1)}
  },[bets,config])
  ...
  const kelly = useMemo(()=>{
    ...
    const avgOdd=fin.reduce((s,b)=>s+b.odd,0)/fin.length
    const b=avgOdd-1
    const kf=(b*p-(1-p))/b
    return {kf:(kf*100).toFixed(1),val:Math.max(0,kf*config.bankroll_inicial).toFixed(2)}
  },[bets,config])

  return (
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      {stats.emRisco&&(
        <div style={{background:'#ff174422',border:'1px solid #ff174466',borderRadius:14,padding:'16px 20px',display:'flex',alignItems:'center',gap:12}}>
          <span style={{fontSize:24}}>⚠️</span>
          <div>
            <div style={{color:'#ff5252',fontWeight:700,fontSize:15}}>Atenção ao Stop Loss!</div>
      ...
      {kelly&&(
        <div style={{background:'#111724',border:'1px solid #ffab0033',borderRadius:16,padding:'20px 24px'}}>
          <div style={{fontWeight:700,fontSize:15,marginBottom:4}}>🎯 Sugestão — Critério de Kelly</div>
          <div style={{color:'#8892a4',fontSize:12,marginBottom:14}}>Baseado no seu histórico de apostas finalizadas</div>
          <div style={{display:'flex',gap:24,flexWrap:'wrap'}}>
            <div><div style={{fontSize:11,color:'#8892a4',marginBottom:4}}>% DO BANKROLL</div><div style={{fontSize:26,fontWeight:800,color:'#ffab00',fontFamily:"'Bebas Neue',cursive"}}>{kelly.kf}%</div></div>
            <div><div style={{fontSize:11,color:'#8892a4',marginBottom:4}}>VALOR SUGERIDO</div><div style={{fontSize:26,fontWeight:800,color:'#ffab00',fontFamily:"'Bebas Neue',cursive"}}>R$ {kelly.val}</div></div>
          </div>
          <div style={{color:'#8892a4',fontSize:11,marginTop:10}}>⚠️ Sugestão teórica. Nunca aposte mais do que pode perder.</div>
        </div>
      )}
      ...
      <div style={{background:'#111724',border:'1px solid #1e2538',borderRadius:16,padding:'20px 24px'}}>
        ...
        {editing?(
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
            <div>
              <label style={{fontSize:11,color:'#8892a4',fontWeight:700,display:'block',marginBottom:5}}>BANKROLL INICIAL (R$)</label>
              <input type="number" value={form.bankroll_inicial} onChange={e=>setForm(f=>({...f,bankroll_inicial:e.target.value}))} style={inp} />
            </div>
            <div>
              <label style={{fontSize:11,color:'#8892a4',fontWeight:700,display:'block',marginBottom:5}}>STOP LOSS (%)</label>
              <input type="number" value={form.stop_loss_percent} onChange={e=>setForm(f=>({...f,stop_loss_percent:e.target.value}))} style={inp} min="1" max="100" />
            </div>
...
function ApostasTab({ bets, userId, onRefresh }) {
  ...
  async function updateStatus(id,status) {
    ...
    await supabase.from('apostas').update({status,retorno}).eq('id',id); onRefresh()
  }

  async function deleteBet(id) {
    if(!confirm('Excluir esta aposta?')) return
    await supabase.from('apostas').delete().eq('id',id); onRefresh()
  }

  const counts = useMemo(()=>({
    todos:bets.length, pendente:bets.filter(b=>b.status==='pendente').length,
    ganhou:bets.filter(b=>b.status==='ganhou').length, perdeu:bets.filter(b=>b.status==='perdeu').length,
  }),[bets])

  const filtered = useMemo(()=>{
    let list=filter==='todos'?bets:bets.filter(b=>b.status===filter)
    if(search) list=list.filter(b=>b.evento?.toLowerCase().includes(search.toLowerCase())||b.selecao?.toLowerCase().includes(search.toLowerCase())||b.casa?.toLowerCase().includes(search.toLowerCase()))
    return [...list].sort((a,b)=>{ let av=a[sortField],bv=b[sortField]; if(typeof av==='string') return sortDir==='asc'?av.localeCompare(bv):bv.localeCompare(av); return sortDir==='asc'?av-bv:bv-av })
  ...
  return (
    <>
      <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
        ...
        <input placeholder="Buscar evento, seleção, casa..." value={search} onChange={e=>setSearch(e.target.value)} style={{...inp,width:220,marginLeft:'auto'}} />
          ...
          <thead>
            <tr style={{background:'#0f1320',borderBottom:'1px solid #1e2538'}}>
              {[['data','Data'],['evento','Evento'],['esporte','Esporte'],['casa','Casa'],['odd','Odd'],['valor','Valor'],['status','Status'],['retorno','Retorno']].map(([f,l])=>(
                <th key={f} onClick={()=>hs(f)} style={{padding:'11px 14px',textAlign:'left',fontSize:10,fontWeight:700,color:'#8892a4',letterSpacing:1,textTransform:'uppercase',cursor:'pointer',whiteSpace:'nowrap',userSelect:'none'}}>
...
function BetApp({ user }) {
  ...
  const stats = useMemo(()=>{
    ...
    return {total:bets.length,won:won.length,lost:lost.length,pending:bets.filter(b=>b.status==='pendente').length,invested,profit,roi,winrate}
  },[bets])
