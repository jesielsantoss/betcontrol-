...
const STATUS_COLORS = { ganhou: '#00e600', perdeu: '#ff1744', pendente: '#f5c518' }
...
const CHART_COLORS = ['#33dd33','#00e600','#f5c518','#ff1744','#00bcd4','#e040fb']

const inp = { background:'#1a1e1a',border:'1px solid #2d3a2d',borderRadius:8,color:'#ffffff',padding:'9px 12px',fontSize:14,outline:'none',width:'100%',fontFamily:'inherit' }

// ── Estilos de botao reutilizaveis ──
const btnPrimary = { background:'linear-gradient(135deg,#00c800,#006600)',border:'none',color:'#fff',borderRadius:8,padding:'10px 22px',cursor:'pointer',fontWeight:700,fontSize:13,boxShadow:'0 4px 14px #00c80033' }
const btnGhost   = { background:'#1e261e',border:'1px solid #00c80033',color:'#33dd33',borderRadius:8,padding:'8px 14px',fontWeight:600,fontSize:12,cursor:'pointer' }
...
function showToast(msg, type='success') {
  ...
  if (!el) return
  el.textContent = msg
  el.style.background = type==='success'?'#00e60022':type==='error'?'#ff174422':'#f5c51822'
  el.style.borderColor = type==='success'?'#00c800':type==='error'?'#ff1744':'#f5c518'
  el.style.color = type==='success'?'#00e600':type==='error'?'#ff5252':'#f5c518'
...
function StatCard({ label, value, sub, color, trend, onClick }) {
  return (
    <div onClick={onClick} style={{ background:'linear-gradient(135deg,#13182a 0%,#1a1f32 100%)',border:`1px solid ${color}33`,borderRadius:18,padding:'20px 24px',flex:1,minWidth:140,position:'relative',overflow:'hidden',cursor:onClick?'pointer':'default',transition:'transform 0.15s,box-shadow 0.15s',boxShadow:`0 4px 20px ${color}0a` }}>
      ...
      <div style={{ color:'#8a9e8a',fontSize:10,fontWeight:700,letterSpacing:1.5,textTransform:'uppercase',marginBottom:8 }}>{label}</div>
      <div style={{ color,fontSize:24,fontWeight:900,fontFamily:"'Bebas Neue',cursive",letterSpacing:1 }}>{value}</div>
      {sub && <div style={{ color:'#8a9e8a',fontSize:11,marginTop:4 }}>{sub}</div>}
      {trend !== undefined && <div style={{ fontSize:11,marginTop:4,color:trend>=0?'#00e600':'#ff1744',fontWeight:700 }}>{trend>=0?'+':''}{trend}% vs mes anterior</div>}
    </div>
  )
}

function ProbBar({ label, value, color='#33dd33' }) {
  return (
    <div style={{marginBottom:12}}>
      ...
      <div style={{background:'#222822',borderRadius:6,height:7}}>
...
function GlassCard({ children, style={}, glow }) {
  return (
    <div style={{
      background:'linear-gradient(135deg,#222822 0%,#1e261e 100%)',
      border:`1px solid ${glow?glow+'33':'#222822'}`,
      borderRadius:18,
      padding:'22px 26px',
...
function DashboardTab({ bets, onNewBet, onTabChange }) {
  const tt = {background:'#1e261e',border:'1px solid #2d3a2d',borderRadius:8,color:'#ffffff',fontSize:12}
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:22}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:14}}>
        <StatCard label="Lucro Total" value={`R$ ${stats.profit.toFixed(2)}`} color={stats.profit>=0?'#00e600':'#ff1744'} trend={parseFloat(stats.roi)}/>
        <StatCard label="ROI" value={`${stats.roi}%`} color={parseFloat(stats.roi)>=0?'#00e600':'#ff1744'}/>
        <StatCard label="Taxa de Acerto" value={`${stats.winrate}%`} sub={`${stats.won} ganhas / ${stats.lost} perdidas`} color="#f5c518"/>
        <StatCard label="Apostas" value={stats.total} sub={`${stats.pending} pendentes`} color="#33dd33"/>
      </div>

      {/* Streak + Acao rapida */}
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        <GlassCard glow={stats.streak.type==='ganhou'?'#00e600':stats.streak.type==='perdeu'?'#ff1744':undefined}>
          <div style={{fontSize:11,color:'#8a9e8a',fontWeight:700,letterSpacing:1,marginBottom:10}}>SEQUENCIA ATUAL</div>
          {stats.streak.count>0 ? (
            <div style={{display:'flex',alignItems:'center',gap:14}}>
              <div style={{fontSize:52,fontWeight:900,fontFamily:"'Bebas Neue',cursive",color:stats.streak.type==='ganhou'?'#00e600':'#ff1744',lineHeight:1}}>{stats.streak.count}</div>
              <div>
                <div style={{fontSize:15,fontWeight:700,color:stats.streak.type==='ganhou'?'#00e600':'#ff1744'}}>{stats.streak.type==='ganhou'?'Vitorias':'Derrotas'} seguidas</div>
                <div style={{fontSize:11,color:'#8a9e8a',marginTop:3}}>{stats.streak.type==='ganhou'?'Continue o ritmo!':'Cuidado, avalie suas entradas'}</div>
              </div>
            </div>
          ) : <div style={{color:'#8a9e8a',fontSize:13}}>Sem apostas finalizadas ainda</div>}
        </GlassCard>

        <GlassCard>
          <div style={{fontSize:11,color:'#8a9e8a',fontWeight:700,letterSpacing:1,marginBottom:12}}>ACOES RAPIDAS</div>
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            <button onClick={onNewBet} style={{background:'linear-gradient(135deg,#00c800,#007700)',border:'none',color:'#fff',borderRadius:10,padding:'11px 16px',fontWeight:700,fontSize:13,cursor:'pointer',textAlign:'left'}}>+ Registrar nova aposta</button>
            <div style={{display:'flex',gap:8}}>
              <button onClick={()=>onTabChange('sugestoes')} style={{flex:1,background:'#1e261e',border:'1px solid #00c80033',color:'#33dd33',borderRadius:10,padding:'9px 12px',fontWeight:600,fontSize:12,cursor:'pointer'}}>Ver Sugestoes</button>
              <button onClick={()=>onTabChange('scouts')} style={{flex:1,background:'#1e261e',border:'1px solid #00c80033',color:'#33dd33',borderRadius:10,padding:'9px 12px',fontWeight:600,fontSize:12,cursor:'pointer'}}>Scouts</button>
            </div>
          </div>
        </GlassCard>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1.5fr 1fr',gap:16}}>
        <GlassCard>
          ...
          {bankrollData.length < 3
            ? <div style={{color:'#8a9e8a',textAlign:'center',padding:40,fontSize:13}}>Registre mais apostas para ver o grafico</div>
            : <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={bankrollData}>
                  <defs>
                    <linearGradient id="lg" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#00e600" stopOpacity={0.3}/>
                      <stop offset="95%" stopColor="#00e600" stopOpacity={0}/>
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="3 3" stroke="#222822"/>
                  <XAxis dataKey="data" stroke="#8a9e8a" fontSize={10}/>
                  <YAxis stroke="#8a9e8a" fontSize={10} tickFormatter={v=>`R$${v}`}/>
                  <Tooltip contentStyle={tt} formatter={v=>[`R$ ${v}`,'Lucro']}/>
                  <Area type="monotone" dataKey="lucro" stroke="#00e600" strokeWidth={2.5} fill="url(#lg)" dot={false}/>
                </AreaChart>
              </ResponsiveContainer>}
        </GlassCard>
        ...
        <GlassCard>
          ...
          {ultimas.length===0
            ? <div style={{color:'#8a9e8a',fontSize:13,textAlign:'center',padding:20}}>Nenhuma aposta ainda</div>
            : <div style={{display:'flex',flexDirection:'column',gap:8}}>
                {ultimas.map(b=>(
                  <div key={b.id} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 10px',background:'#1a1e1a',borderRadius:10}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:10,color:'#8a9e8a'}}>{b.data} · odd {Number(b.odd).toFixed(2)}</div>
                    </div>
                    <div style={{fontSize:12,fontWeight:700,color:b.status==='ganhou'?'#00e600':b.status==='perdeu'?'#ff1744':'#f5c518',flexShrink:0}}>
      ...
      {porEsporte.length > 0 && (
        <GlassCard>
          ...
          <div style={{display:'flex',gap:12,flexWrap:'wrap'}}>
            {porEsporte.map((e,i)=>(
              <div key={e.name} style={{background:'#1a1e1a',border:`1px solid ${CHART_COLORS[i]}33`,borderRadius:14,padding:'14px 20px',flex:1,minWidth:120,textAlign:'center'}}>
                <div style={{fontSize:22,fontWeight:900,color:CHART_COLORS[i],fontFamily:"'Bebas Neue',cursive"}}>{e.value}</div>
                <div style={{fontSize:11,color:'#8a9e8a',marginTop:3}}>{e.name}</div>
              </div>
            ))}
          </div>
        </GlassCard>
    ...
    return (
      <div key={label} style={{marginBottom:24}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10,flexWrap:'wrap',gap:8}}>
          <span style={{fontSize:12,fontWeight:700,color:'#33dd33',letterSpacing:0.5}}>{label}</span>
          <div style={{display:'flex',gap:8,alignItems:'center'}}>
            <span style={{fontSize:10,color:'#a0aec0',background:'#33dd3311',border:'1px solid #33dd3322',borderRadius:5,padding:'2px 8px'}}>Fair Odd: <strong style={{color:'#fff'}}>{fairOdd}</strong></span>
            {diferenca > 0 && <span style={{fontSize:10,color:'#f5c518',background:'#f5c51811',border:'1px solid #f5c51822',borderRadius:5,padding:'2px 8px'}}>Spread: {diferenca}%</span>}
          </div>
        </div>
          ...
          {sorted.map((item,i)=>{
            ...
            return (
              <div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',
                background:hasValue?'#00e60006':isMelhor?'#ffffff04':'#1a1e1a',
                border:`1px solid ${hasValue?'#00e60044':isMelhor?'#2a3a5a':'#222822'}`,
                borderRadius:11,transition:'all 0.15s',position:'relative'}}>
                {hasValue&&<div style={{position:'absolute',top:-1,right:10,background:'#00e600',color:'#001a00',fontSize:9,fontWeight:900,padding:'1px 8px',borderRadius:'0 0 6px 6px',letterSpacing:0.5}}>VALUE BET</div>}
                {isMelhor&&!hasValue&&<div style={{width:4,height:40,background:'#33dd33',borderRadius:2,flexShrink:0,boxShadow:'0 0 10px #33dd3377'}}/>}
                {hasValue&&<div style={{width:4,height:40,background:'#00e600',borderRadius:2,flexShrink:0,boxShadow:'0 0 12px #00e60099'}}/>}
                <div style={{flex:1}}>
                  <div style={{fontSize:14,fontWeight:700,color:hasValue?'#00e600':isMelhor?'#ffffff':'#a0aec0'}}>{item.casa}</div>
                  <div style={{fontSize:10,color:'#8a9e8a',marginTop:1}}>
                    Prob impl: {(100/item.odd).toFixed(1)}% | Fair: {(probMedia).toFixed(1)}%
                  </div>
                </div>
                {item.href&&<a href={item.href} target="_blank" rel="noreferrer" style={{fontSize:10,color:'#00c800',border:'1px solid #00c80044',borderRadius:6,padding:'3px 9px',textDecoration:'none',fontWeight:700,flexShrink:0}}>Apostar</a>}
                <div style={{textAlign:'center',minWidth:56}}>
                  <div style={{fontSize:26,fontWeight:900,color:hasValue?'#00e600':isMelhor?'#fff':'#f5c518',fontFamily:"'Bebas Neue',cursive",lineHeight:1}}>{item.odd.toFixed(2)}</div>
                  {item.odd > fairOdd && <div style={{fontSize:9,color:'#00e600',fontWeight:700}}>+{((item.odd-fairOdd)/fairOdd*100).toFixed(1)}% vs fair</div>}
                  {item.odd <= fairOdd && <div style={{fontSize:9,color:'#8a9e8a'}}>{((item.odd-fairOdd)/fairOdd*100).toFixed(1)}% vs fair</div>}
                </div>
                <div style={{textAlign:'right',minWidth:90}}>
                  <div style={{fontSize:13,fontWeight:800,color:ev>0.05?'#00e600':ev>0?'#69ff84':ev>-0.03?'#f5c518':'#ff5252'}}>
                    EV {ev>0?'+':''}{(ev*100).toFixed(1)}%
                  </div>
                  {hasValue
                    ?<div style={{fontSize:10,color:'#00e600',fontWeight:700,marginTop:1}}>🔥 APOSTAR</div>
                    :isMelhor?<div style={{fontSize:10,color:'#33dd33',marginTop:1}}>MELHOR</div>
                    :<div style={{fontSize:10,color:'#8a9e8a',marginTop:1}}>-{diffVsMelhor}% vs melhor</div>}
                </div>
              </div>
            )
          })}
        </div>
        <div style={{marginTop:8,padding:'6px 12px',background:'#ffffff06',borderRadius:7,fontSize:10,color:'#8a9e8a'}}>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      ...
      <GlassCard>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
          <div>
            ...
            <div style={{color:'#8a9e8a',fontSize:12,marginTop:2}}>Odds reais de 250+ casas via odds-api.io</div>
          </div>
          <button onClick={testarAPI} disabled={testando} style={{background:'#1e261e',border:'1px solid #00c80044',color:'#33dd33',borderRadius:8,padding:'7px 14px',cursor:'pointer',fontSize:12,fontWeight:700}}>
            {testando?'Testando...':'Testar Conexao'}
          </button>
        </div>
        {testeResult&&(
          <div style={{marginTop:10,padding:'8px 12px',background:testeResult.ok?'#00e60011':'#ff174411',border:`1px solid ${testeResult.ok?'#00e60033':'#ff174433'}`,borderRadius:8,fontSize:12,color:testeResult.ok?'#00e600':'#ff7070'}}>
            {testeResult.msg}
          </div>
        )}
      </GlassCard>
      ...
      <GlassCard>
        <div style={{display:'grid',gridTemplateColumns:'1fr auto',gap:10,alignItems:'end'}}>
          <div>
            <label style={{fontSize:11,color:'#8a9e8a',fontWeight:700,display:'block',marginBottom:5}}>BUSCAR PARTIDA</label>
            <input
          ...
          <button onClick={buscarEventos} disabled={loadingEventos} style={{background:'linear-gradient(135deg,#00c800,#009000)',border:'none',color:'#fff',borderRadius:8,padding:'10px 22px',cursor:'pointer',fontWeight:700,fontSize:13,boxShadow:'0 4px 14px #00c80033',whiteSpace:'nowrap'}}>
        ...
        {!eventoSel&&eventos.length>0&&(
          <div style={{marginTop:14,display:'flex',flexDirection:'column',gap:7}}>
            <div style={{fontSize:11,color:'#8a9e8a',fontWeight:700,marginBottom:4}}>SELECIONE O JOGO</div>
            {eventos.map(ev=>(
              <button key={ev.id} onClick={()=>buscarOdds(ev)}
                style={{background:'#1a1e1a',border:'1px solid #222822',borderRadius:10,padding:'12px 16px',cursor:'pointer',textAlign:'left',display:'flex',justifyContent:'space-between',alignItems:'center',color:'#ffffff',transition:'all 0.15s'}}
                onMouseEnter={e=>e.currentTarget.style.borderColor='#00c800'}
                onMouseLeave={e=>e.currentTarget.style.borderColor='#222822'}>
                <div>
                  <div style={{fontSize:13,fontWeight:700}}>{ev.home} <span style={{color:'#8a9e8a',fontWeight:400}}>x</span> {ev.away}</div>
                  <div style={{fontSize:11,color:'#8a9e8a',marginTop:2}}>{ev.league||ev.sport||''}{ev.date?' · '+new Date(ev.date).toLocaleDateString('pt-BR'):''}</div>
                </div>
                <span style={{fontSize:11,color:'#00c800',fontWeight:700,flexShrink:0}}>Ver odds</span>
              </button>
            ))}
          </div>
        )}
        ...
        {eventoSel&&(
          <div style={{marginTop:12,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 14px',background:'#1e261e',border:'1px solid #00c80044',borderRadius:10}}>
            <div>
              <div style={{fontSize:14,fontWeight:700,color:'#33dd33'}}>{eventoSel.home} x {eventoSel.away}</div>
              <div style={{fontSize:11,color:'#8a9e8a'}}>{eventoSel.league}</div>
            </div>
            <button onClick={()=>{setEventoSel(null);setOddsData(null);setEventos([])}} style={{background:'transparent',border:'none',color:'#8a9e8a',cursor:'pointer',fontSize:12,fontWeight:600}}>Trocar jogo</button>
          </div>
        )}
      </GlassCard>

      {loadingOdds&&<div style={{textAlign:'center',padding:40,color:'#8a9e8a',fontSize:13}}>Buscando odds em tempo real...</div>}
        ...
        <>
          ...
          <GlassCard>
            <div style={{fontWeight:700,fontSize:15,marginBottom:14}}>
              ...
              <span style={{fontSize:11,color:'#8a9e8a',fontWeight:400,marginLeft:10}}>{Object.keys(oddsData.bookmakers||{}).length} casas</span>
            </div>
            <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:20}}>
              {mercadosNomes.map(mk=>(
                <button key={mk} onClick={()=>setMercadoFiltro(mk)}
                  style={{background:mercadoAtivo===mk?'#1e2a4a':'transparent',border:`1px solid ${mercadoAtivo===mk?'#00c800':'#2d3a2d'}`,color:mercadoAtivo===mk?'#fff':'#8a9e8a',borderRadius:8,padding:'6px 14px',fontSize:12,fontWeight:600,cursor:'pointer'}}>
                  {mk}
                </button>
              ))}
            </div>
            {Object.entries(grupos).map(([label,items])=>renderGrupo(label,items))}
            <div style={{marginTop:8,padding:'10px 14px',background:'#33dd3311',border:'1px solid #33dd3322',borderRadius:8}}>
              <div style={{fontSize:11,color:'#a0aec0'}}><strong style={{color:'#33dd33'}}>EV positivo</strong> = odd acima da probabilidade media do mercado. Dados ao vivo via odds-api.io.</div>
            </div>
          </GlassCard>
        </>
      )}
      ...
      {oddsData&&mercadosNomes.length===0&&!loadingOdds&&(
        <GlassCard><div style={{color:'#f5c518',fontSize:13,textAlign:'center',padding:20}}>Odds retornadas mas sem mercados reconhecidos. Tente outro jogo.</div></GlassCard>
      )}
    </div>
  )
}
  ...
  async function buscarSugestoes() {
    ...
    try {
      ...
      for (const event of todos.slice(0, 30)) {
        ...
        const confCor = melhor.prob >= 65 ? '#00e600' : melhor.prob >= 55 ? '#f5c518' : '#ff5252'
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      <GlassCard>
        ...
        <div style={{color:'#8a9e8a',fontSize:12,marginBottom:18}}>
          Probabilidades calculadas via Distribuicao de Poisson (λ por time). Fair Odds sem margem das casas.
        </div>
        <div style={{display:'grid',gridTemplateColumns:'1fr auto',gap:10,alignItems:'end'}}>
          <div>
            <label style={{fontSize:11,color:'#8a9e8a',fontWeight:700,display:'block',marginBottom:5}}>COMPETICAO</label>
          ...
          <button onClick={buscarSugestoes} disabled={loading} style={{background:'linear-gradient(135deg,#00c800,#009000)',border:'none',color:'#fff',borderRadius:8,padding:'10px 22px',cursor:'pointer',fontWeight:700,fontSize:14,whiteSpace:'nowrap',boxShadow:'0 4px 16px #00c80044'}}>
            {loading ? 'Calculando...' : 'Analisar Jogos'}
          </button>
        </div>

        {/* Legenda Poisson */}
        <div style={{marginTop:14,padding:'10px 14px',background:'#00c80011',border:'1px solid #00c80022',borderRadius:8,fontSize:11,color:'#a0aec0'}}>
          <strong style={{color:'#33dd33'}}>Modelo Poisson:</strong> P(k;λ) = λᵏ·e⁻λ / k! — calcula a probabilidade exata de cada placar possível e agrega em Over/Under, BTTS e resultado.
          <strong style={{color:'#f5c518'}}> Fair Odd</strong> = odd justa sem margem da casa. Se a casa pagar acima da fair odd → <strong style={{color:'#00e600'}}>Value Bet</strong>.
        </div>
      </GlassCard>

      {searched && !loading && sugestoes.length === 0 && (
        <div style={{textAlign:'center',color:'#8a9e8a',padding:50,background:'#222822',borderRadius:16,border:'1px solid #222822'}}>
      ...
      {sugestoes.length > 0 && (
        <>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
            {[['Alta','alta','#00e600'],['Media','media','#f5c518'],['Baixa','baixa','#ff5252']].map(([lbl,val,cor])=>{
              const count = sugestoes.filter(s=>s.confianca===lbl).length
              return (
                <button key={lbl} onClick={()=>setFiltro(filtro===val?'todos':val)}
                  style={{background:filtro===val?cor+'18':'#222822',border:`1px solid ${filtro===val?cor:cor+'33'}`,borderRadius:14,padding:'14px 18px',textAlign:'center',cursor:'pointer',transition:'all 0.15s'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            {filtradas.map(s => (
              <GlassCard key={s.id} glow={s.confCor} style={{padding:'0',overflow:'hidden'}}>
                {/* Header */}
                <div style={{padding:'18px 22px',borderBottom:'1px solid #222822',display:'flex',alignItems:'center',gap:12}}>
                  <img src={s.homeLogo} style={{width:36,height:36,objectFit:'contain'}} alt="" onError={e=>e.target.style.display='none'}/>
                  <div style={{flex:1}}>
                    <div style={{fontSize:15,fontWeight:800}}>{s.homeName} <span style={{color:'#8a9e8a',fontWeight:400}}>x</span> {s.awayName}</div>
                    <div style={{fontSize:11,color:'#8a9e8a',marginTop:2}}>{s.data} · {s.hora} · λ={+(s.lambdaH+s.lambdaA).toFixed(2)} gols esperados</div>
                  </div>
                ...
                <div style={{padding:'14px 22px',borderBottom:'1px solid #222822',background:'#0a0e1a'}}>
                  {/* Resultado 1X2 */}
                  <div style={{display:'grid',gridTemplateColumns:'1fr auto 1fr',gap:8,marginBottom:14,alignItems:'center'}}>
                    <div style={{textAlign:'center',background:'#1a1e1a',borderRadius:10,padding:'10px 6px'}}>
                      <div style={{fontSize:10,color:'#f5c518',fontWeight:700,marginBottom:2}}>{s.homeShort}</div>
                      <div style={{fontSize:24,fontWeight:900,color:'#f5c518',fontFamily:"'Bebas Neue',cursive"}}>{s.poiss.homeWin}%</div>
                      <div style={{fontSize:10,color:'#8a9e8a'}}>Fair: {s.fairHome}</div>
                      <div style={{fontSize:9,color:'#8a9e8a'}}>λ={s.lambdaH}</div>
                    </div>
                    <div style={{textAlign:'center',background:'#1a1e1a',borderRadius:10,padding:'10px 8px'}}>
                      <div style={{fontSize:10,color:'#8a9e8a',fontWeight:700,marginBottom:2}}>EMPATE</div>
                      <div style={{fontSize:24,fontWeight:900,color:'#8a9e8a',fontFamily:"'Bebas Neue',cursive"}}>{s.poiss.draw}%</div>
                      <div style={{fontSize:10,color:'#8a9e8a'}}>Fair: {s.fairDraw}</div>
                    </div>
                    <div style={{textAlign:'center',background:'#1a1e1a',borderRadius:10,padding:'10px 6px'}}>
                      <div style={{fontSize:10,color:'#33dd33',fontWeight:700,marginBottom:2}}>{s.awayShort}</div>
                      <div style={{fontSize:24,fontWeight:900,color:'#33dd33',fontFamily:"'Bebas Neue',cursive"}}>{s.poiss.awayWin}%</div>
                      <div style={{fontSize:10,color:'#8a9e8a'}}>Fair: {s.fairAway}</div>
                      <div style={{fontSize:9,color:'#8a9e8a'}}>λ={s.lambdaA}</div>
                    </div>
                  </div>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8}}>
                    {[
                      ...
                      ['Over 2.5',s.poiss.over25,'#00e600'],
                      ['Over 3.5',s.poiss.over35,'#f5c518'],
                      ['BTTS',s.poiss.btts,'#33dd33'],
                    ].map(([lbl,prob,cor])=>(
                      <div key={lbl} style={{background:'#1a1e1a',borderRadius:8,padding:'8px 6px',textAlign:'center'}}>
                        ...
                        <div style={{background:'#222822',borderRadius:3,height:3,marginTop:4}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginTop:12}}>
                    {[[s.homeShort,s.hr,'Casa'],[s.awayShort,s.ar,'Fora']].map(([nome,rec,tipo])=>(
                      <div key={tipo} style={{display:'flex',gap:6,alignItems:'center',background:'#1a1e1a',borderRadius:8,padding:'8px 10px'}}>
                        <span style={{fontSize:10,color:'#8a9e8a',fontWeight:700,minWidth:30}}>{tipo}</span>
                        <span style={{fontSize:11,color:'#ffffff',fontWeight:700,flex:1}}>{nome}</span>
                        {[['V',rec.w,'#00e600'],['E',rec.d,'#f5c518'],['D',rec.l,'#ff1744']].map(([l,v,c])=>(
                ...
                <div style={{padding:'14px 22px',display:'flex',gap:10,flexWrap:'wrap'}}>
                  {s.bets.map((bet,i)=>(
                    <div key={i} style={{background:'#1a1e1a',border:`1px solid ${bet.cor}22`,borderRadius:12,padding:'12px 16px',flex:1,minWidth:160}}>
                      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                        ...
                        <span style={{fontSize:12,fontWeight:700,color:'#ffffff'}}>{bet.tipo}</span>
                      </div>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',marginBottom:6}}>
                        <div>
                          <div style={{fontSize:9,color:'#8a9e8a'}}>PROBABILIDADE</div>
                          <div style={{fontSize:26,fontWeight:900,color:bet.cor,fontFamily:"'Bebas Neue',cursive",lineHeight:1}}>{bet.prob}%</div>
                        </div>
                        <div style={{textAlign:'right'}}>
                          <div style={{fontSize:9,color:'#8a9e8a'}}>FAIR ODD</div>
                          <div style={{fontSize:20,fontWeight:800,color:'#f5c518',fontFamily:"'Bebas Neue',cursive",lineHeight:1}}>{bet.fairOdd}</div>
                        </div>
                      </div>
                      <div style={{background:'#222822',borderRadius:4,height:4,marginBottom:8}}>
                      ...
                      <div style={{fontSize:10,color:'#8a9e8a',marginBottom:6}}>{bet.motivo}</div>
                      <div style={{fontSize:10,color:'#a0aec0',padding:'4px 8px',background:'#1e261e',borderRadius:6}}>
                        Se a casa pagar <strong style={{color:'#00e600'}}>{'>'}{bet.fairOdd}</strong> → <strong style={{color:'#00e600'}}>Value Bet!</strong>
                      </div>
                    </div>
                  ))}
                </div>
...
function FormaRecente({ record, cor }) {
  if (!record) return <span style={{fontSize:10,color:'#8a9e8a'}}>--</span>
  const parts = record.split('-').map(Number)
  const w=parts[0]||0, d=parts[1]||0, l=parts[2]||0
  // gerar forma simulada baseada no record
  const total = w+d+l
  if (total===0) return <span style={{fontSize:10,color:'#8a9e8a'}}>--</span>
  const forma = []
  ...
  const cores = {V:'#00e600',E:'#f5c518',D:'#ff4444'}
  return (
...
function ScoutCard({ event, ligaNome, onSelect, isSelected }) {
  ...
  const tendencia = over25>=60 ? {label:'Over 2.5',cor:'#00e600',icone:'GOL'} : btts>=58 ? {label:'BTTS',cor:'#33dd33',icone:'BTTS'} : homeWin>=60 ? {label:'Casa Vence',cor:'#f5c518',icone:'CASA'} : awayWin>=55 ? {label:'Fora Vence',cor:'#e040fb',icone:'FORA'} : {label:'Indefinido',cor:'#8a9e8a',icone:'?'}
  ...
  return (
    <div
      ...
      style={{
        background: active ? 'linear-gradient(135deg,#141e38,#1a2440)' : 'linear-gradient(135deg,#222822,#13192a)',
        border: `1px solid ${active ? '#00c80055' : '#22282288'}`,
        borderRadius:16,
        padding:'18px 20px',
        cursor:'pointer',
        transition:'all 0.2s ease',
        boxShadow: active ? '0 4px 24px #00c80018' : '0 2px 8px #00000022',
        position:'relative',
        overflow:'hidden',
      }}
    >
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
        <span style={{fontSize:10,color:'#8a9e8a',fontWeight:700,letterSpacing:1}}>{ligaNome.toUpperCase()}</span>
        {aoVivo
          ...
          : encerrado
          ? <span style={{fontSize:11,color:'#8a9e8a',fontWeight:600}}>Encerrado</span>
          : <span style={{fontSize:13,fontWeight:700,color:'#f5c518'}}>{hora}</span>}
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr auto 1fr',gap:12,alignItems:'center',marginBottom:14}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:6}}>
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            ...
            <div>
              ...
              <div style={{fontSize:10,color:'#8a9e8a',marginTop:2}}>{hr.w}V {hr.d}E {hr.l}D</div>
            </div>
          </div>
        ...
        <div style={{textAlign:'center',minWidth:48}}>
          {(aoVivo||encerrado)&&home.score!==undefined
            ? <div style={{fontSize:24,fontWeight:900,fontFamily:"'Bebas Neue',cursive",letterSpacing:2,color:aoVivo?'#fff':'#8a9e8a'}}>{home.score}<span style={{color:'#2d3a2d',fontSize:18}}> - </span>{away.score}</div>
            : <div style={{fontSize:14,fontWeight:700,color:'#2d3a2d'}}>VS</div>}
        </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:6,alignItems:'flex-end'}}>
          <div style={{display:'flex',alignItems:'center',gap:8,flexDirection:'row-reverse'}}>
            ...
            <div style={{textAlign:'right'}}>
              ...
              <div style={{fontSize:10,color:'#8a9e8a',marginTop:2}}>{ar.w}V {ar.d}E {ar.l}D</div>
            </div>
          </div>
      ...
      {/* Stats rapidas */}
      <div style={{display:'flex',gap:8,borderTop:'1px solid #22282255',paddingTop:12}}>
        <div style={{flex:1,textAlign:'center',background:'#1a1e1a88',borderRadius:8,padding:'7px 4px'}}>
          <div style={{fontSize:9,color:'#8a9e8a',fontWeight:700,letterSpacing:0.5,marginBottom:3}}>OVER 2.5</div>
          <div style={{fontSize:15,fontWeight:900,color:over25>=60?'#00e600':over25>=50?'#f5c518':'#ff5252',fontFamily:"'Bebas Neue',cursive"}}>{over25}%</div>
        </div>
        <div style={{flex:1,textAlign:'center',background:'#1a1e1a88',borderRadius:8,padding:'7px 4px'}}>
          <div style={{fontSize:9,color:'#8a9e8a',fontWeight:700,letterSpacing:0.5,marginBottom:3}}>BTTS</div>
          <div style={{fontSize:15,fontWeight:900,color:btts>=60?'#33dd33':btts>=50?'#f5c518':'#8a9e8a',fontFamily:"'Bebas Neue',cursive"}}>{btts}%</div>
        </div>
        <div style={{flex:1,textAlign:'center',background:'#1a1e1a88',borderRadius:8,padding:'7px 4px'}}>
          <div style={{fontSize:9,color:'#8a9e8a',fontWeight:700,letterSpacing:0.5,marginBottom:3}}>CASA</div>
          <div style={{fontSize:15,fontWeight:900,color:'#f5c518',fontFamily:"'Bebas Neue',cursive"}}>{homeWin}%</div>
        </div>
        <div style={{flex:1,textAlign:'center',background:'#1a1e1a88',borderRadius:8,padding:'7px 4px'}}>
          <div style={{fontSize:9,color:'#8a9e8a',fontWeight:700,letterSpacing:0.5,marginBottom:3}}>FORA</div>
          <div style={{fontSize:15,fontWeight:900,color:'#33dd33',fontFamily:"'Bebas Neue',cursive"}}>{awayWin}%</div>
        </div>
        {/* Tendencia */}
        <div style={{flex:1.5,textAlign:'center',background:tendencia.cor+'11',border:`1px solid ${tendencia.cor}33`,borderRadius:8,padding:'7px 4px'}}>
          <div style={{fontSize:9,color:'#8a9e8a',fontWeight:700,letterSpacing:0.5,marginBottom:3}}>TENDENCIA</div>
      ...
      {/* Hover hint */}
      {hovered&&!isSelected&&(
        <div style={{position:'absolute',bottom:0,left:0,right:0,background:'linear-gradient(0deg,#00c80022,transparent)',borderRadius:'0 0 16px 16px',padding:'12px',textAlign:'center'}}>
          <span style={{fontSize:11,color:'#33dd33',fontWeight:700}}>Ver scout completo</span>
        </div>
      )}
    </div>
  )
    ...
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      <GlassCard>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr auto auto',gap:10,alignItems:'end'}}>
          ...
          <div>
            <label style={{fontSize:11,color:'#8a9e8a',fontWeight:700,display:'block',marginBottom:5}}>DATA</label>
            <input type="date" value={date} onChange={e=>setDate(e.target.value)} style={{...inp,width:150}}/>
          </div>
          <button onClick={buscar} disabled={loading} style={{background:'linear-gradient(135deg,#00c800,#009000)',border:'none',color:'#fff',borderRadius:8,padding:'10px 20px',cursor:'pointer',fontWeight:700,fontSize:13,boxShadow:'0 4px 14px #00c80033'}}>
      ...
      {loading&&(
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {[1,2,3].map(i=>(
            <div key={i} style={{background:'#222822',border:'1px solid #222822',borderRadius:16,padding:'18px 20px',opacity:0.5}}>
              <div style={{height:12,background:'#222822',borderRadius:6,width:'40%',marginBottom:14}}/>
              <div style={{display:'grid',gridTemplateColumns:'1fr auto 1fr',gap:12,marginBottom:14}}>
                <div style={{height:32,background:'#222822',borderRadius:8}}/>
                <div style={{height:32,width:48,background:'#222822',borderRadius:8}}/>
                <div style={{height:32,background:'#222822',borderRadius:8}}/>
              </div>
              <div style={{height:40,background:'#222822',borderRadius:8}}/>
            </div>
          ))}
        </div>
      )}
      ...
      {searched&&!loading&&events.length===0&&(
        <div style={{textAlign:'center',color:'#8a9e8a',padding:50,background:'#222822',borderRadius:16,border:'1px solid #222822'}}>Nenhuma partida encontrada para esta data.</div>
      )}
        ...
        return (
          <div id="scout-detail" style={{display:'flex',flexDirection:'column',gap:16}}>
            <GlassCard glow="#00c800">
              <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:20}}>
                ...
                <div style={{flex:1,textAlign:'center'}}>
                  ...
                  <div style={{fontSize:11,color:'#8a9e8a',marginTop:3}}>{liga.nome}</div>
              ...
              <div style={{fontWeight:700,fontSize:13,marginBottom:12,color:'#33dd33',letterSpacing:1}}>PROBABILIDADES</div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20}}>
                <div>
                  <ProbBar label={`Vitoria ${home.team?.shortDisplayName||home.team?.displayName}`} value={probs.homeWin} color="#00e600"/>
                  <ProbBar label="Empate" value={probs.draw} color="#f5c518"/>
                  <ProbBar label={`Vitoria ${away.team?.shortDisplayName||away.team?.displayName}`} value={probs.awayWin} color="#ff5252"/>
                </div>
                <div>
                  <ProbBar label="Over 2.5 gols" value={probs.over25} color="#33dd33"/>
            ...
            {stats&&(stats.home.shots!=='0'||stats.home.possession!=='0')&&(
              <GlassCard>
                <div style={{fontWeight:700,fontSize:13,marginBottom:16,color:'#33dd33',letterSpacing:1}}>ESTATISTICAS DA PARTIDA</div>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}>
                  <span style={{fontSize:11,fontWeight:700,color:'#33dd33'}}>{home.team?.shortDisplayName||home.team?.displayName}</span>
                  ...
                  return (
                    <div key={label} style={{marginBottom:10}}>
                      <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                        <span style={{fontSize:13,fontWeight:800,color:'#33dd33'}}>{h}</span>
                        <span style={{fontSize:10,color:'#8a9e8a',fontWeight:600}}>{label}</span>
                        <span style={{fontSize:13,fontWeight:800,color:'#ff5252'}}>{a}</span>
                      </div>
                      <div style={{background:'#222822',borderRadius:6,height:6,display:'flex',overflow:'hidden'}}>
                        <div style={{background:'linear-gradient(90deg,#00c800,#33dd33)',width:`${hpct}%`,transition:'width 0.5s'}}/>
...
function MatchSearch({ onSelect }) {
  ...
  return (
    <div style={{gridColumn:'1 / span 2',marginBottom:4}}>
      <button type="button" onClick={()=>setOpen(o=>!o)} style={{width:'100%',background:'#1a2540',border:'1px solid #00c80044',color:'#33dd33',borderRadius:8,padding:'9px 14px',cursor:'pointer',fontWeight:700,fontSize:13,textAlign:'left',display:'flex',alignItems:'center',gap:8}}>
        Buscar partida para preencher automaticamente <span style={{marginLeft:'auto',fontSize:11,color:'#8a9e8a'}}>{open?'v':'^'}</span>
      </button>
      {open&&(
        <div style={{background:'#1a1e1a',border:'1px solid #2d3a2d',borderRadius:10,padding:14,marginTop:8}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr auto auto',gap:8,marginBottom:10,alignItems:'end'}}>
            <div><label style={{fontSize:10,color:'#8a9e8a',fontWeight:700,display:'block',marginBottom:4}}>COMPETICAO</label><select value={liga.id} onChange={e=>setLiga(LIGAS_ESPN.find(l=>l.id===e.target.value))} style={{...inp,fontSize:12}}>{LIGAS_ESPN.map(l=><option key={l.id} value={l.id}>{l.nome}</option>)}</select></div>
            <div><label style={{fontSize:10,color:'#8a9e8a',fontWeight:700,display:'block',marginBottom:4}}>DATA</label><input type="date" value={date} onChange={e=>setDate(e.target.value)} style={{...inp,fontSize:12,width:140}}/></div>
            <button type="button" onClick={buscar} disabled={loading} style={{background:'linear-gradient(135deg,#00c800,#009000)',border:'none',color:'#fff',borderRadius:8,padding:'9px 16px',cursor:'pointer',fontWeight:700,fontSize:12,whiteSpace:'nowrap'}}>{loading?'...':'Buscar'}</button>
          </div>
          {searched&&!loading&&events.length===0&&<div style={{textAlign:'center',color:'#8a9e8a',padding:16,fontSize:13}}>Nenhuma partida.</div>}
          {events.length>0&&<div style={{display:'flex',flexDirection:'column',gap:6,maxHeight:200,overflowY:'auto'}}>{events.map(event=>{
            const comps=event.competitions?.[0],home=comps?.competitors?.find(c=>c.homeAway==='home'),away=comps?.competitors?.find(c=>c.homeAway==='away')
            return <button type="button" key={event.id} onClick={()=>sel(event)} style={{background:'#1e261e',border:'1px solid #2d3a2d',borderRadius:8,padding:'9px 12px',cursor:'pointer',textAlign:'left',display:'flex',alignItems:'center',gap:10}} onMouseEnter={e=>e.currentTarget.style.borderColor='#00c800'} onMouseLeave={e=>e.currentTarget.style.borderColor='#2d3a2d'}>
              <img src={home?.team?.logo} style={{width:22,height:22,objectFit:'contain'}} alt="" onError={e=>e.target.style.display='none'}/>
              <div style={{flex:1,fontSize:12,fontWeight:600,color:'#ffffff'}}>{home?.team?.displayName} x {away?.team?.displayName}</div>
...
function InteligenciaTab({ bets }) {
  ...
  const insightColors = {positivo:'#00e600',negativo:'#ff1744',alerta:'#f5c518',info:'#33dd33'}
  return (
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      <GlassCard><div style={{fontWeight:700,fontSize:15,marginBottom:16}}>Insights do seu Historico</div><div style={{display:'flex',flexDirection:'column',gap:10}}>{insights.map((ins,i)=><div key={i} style={{background:insightColors[ins.tipo]+'11',border:`1px solid ${insightColors[ins.tipo]}33`,borderRadius:10,padding:'12px 16px'}}><span style={{fontSize:13,color:'#ffffff',lineHeight:1.5}}>{ins.msg}</span></div>)}</div></GlassCard>
      {porMercado.length>0&&<GlassCard><div style={{fontWeight:700,fontSize:15,marginBottom:16}}>ROI por Mercado</div><div style={{display:'flex',flexDirection:'column',gap:8}}>{porMercado.map((m,i)=><div key={i} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',background:'#1a1e1a',borderRadius:10}}><div style={{width:8,height:8,borderRadius:'50%',background:CHART_COLORS[i%CHART_COLORS.length]}}/><div style={{flex:1,fontSize:13,fontWeight:600}}>{m.name}</div><div style={{fontSize:11,color:'#8a9e8a'}}>{m.total} ap.</div><div style={{fontSize:12,color:'#f5c518',width:44,textAlign:'right'}}>{m.winrate}%</div><div style={{fontSize:13,fontWeight:700,color:m.roi>=0?'#00e600':'#ff1744',width:70,textAlign:'right'}}>ROI {m.roi}%</div><div style={{fontSize:13,fontWeight:700,color:m.lucro>=0?'#00e600':'#ff1744',width:90,textAlign:'right'}}>R$ {m.lucro}</div></div>)}</div></GlassCard>}
      {porOddRange.length>0&&<GlassCard><div style={{fontWeight:700,fontSize:15,marginBottom:16}}>Performance por Faixa de Odd</div><ResponsiveContainer width="100%" height={200}><BarChart data={porOddRange}><CartesianGrid strokeDasharray="3 3" stroke="#222822"/><XAxis dataKey="name" stroke="#8a9e8a" fontSize={11}/><YAxis stroke="#8a9e8a" fontSize={11} tickFormatter={v=>`${v}%`}/><Tooltip contentStyle={tt} formatter={(v,n)=>n==='roi'?[`${v}%`,'ROI']:[`${v}%`,'Winrate']}/><Bar dataKey="roi" radius={[4,4,0,0]}>{porOddRange.map((e,i)=><Cell key={i} fill={e.roi>=0?'#00e600':'#ff1744'}/>)}</Bar></BarChart></ResponsiveContainer></GlassCard>}
      <GlassCard><div style={{fontWeight:700,fontSize:15,marginBottom:16}}>Lucro por Dia da Semana</div><ResponsiveContainer width="100%" height={180}><BarChart data={porDia}><CartesianGrid strokeDasharray="3 3" stroke="#222822"/><XAxis dataKey="name" stroke="#8a9e8a" fontSize={12}/><YAxis stroke="#8a9e8a" fontSize={11} tickFormatter={v=>`R$${v}`}/><Tooltip contentStyle={tt} formatter={v=>[`R$ ${v}`,'Lucro']}/><Bar dataKey="lucro" radius={[4,4,0,0]}>{porDia.map((e,i)=><Cell key={i} fill={e.lucro>=0?'#00e600':'#ff1744'}/>)}</Bar></BarChart></ResponsiveContainer></GlassCard>
    </div>
  )
}
...
function Analytics({ bets }) {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      <GlassCard><div style={{fontWeight:700,fontSize:15,marginBottom:16}}>Evolucao do Bankroll</div>{bankrollData.length<2?<div style={{color:'#8a9e8a',textAlign:'center',padding:40}}>Registre mais apostas finalizadas</div>:<ResponsiveContainer width="100%" height={220}><AreaChart data={bankrollData}><defs><linearGradient id="ag" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#00e600" stopOpacity={0.25}/><stop offset="95%" stopColor="#00e600" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="#222822"/><XAxis dataKey="data" stroke="#8a9e8a" fontSize={11}/><YAxis stroke="#8a9e8a" fontSize={11} tickFormatter={v=>`R$${v}`}/><Tooltip contentStyle={tt} formatter={v=>[`R$ ${v}`,'Saldo']}/><Area type="monotone" dataKey="lucro" stroke="#00e600" strokeWidth={2.5} fill="url(#ag)" dot={false}/></AreaChart></ResponsiveContainer>}</GlassCard>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:20}}>
        <GlassCard><div style={{fontWeight:700,fontSize:15,marginBottom:16}}>Lucro por Esporte</div>{byEsporte.length===0?<div style={{color:'#8a9e8a',textAlign:'center',padding:30}}>Sem dados</div>:<ResponsiveContainer width="100%" height={200}><BarChart data={byEsporte} layout="vertical"><CartesianGrid strokeDasharray="3 3" stroke="#222822"/><XAxis type="number" stroke="#8a9e8a" fontSize={11} tickFormatter={v=>`R$${v}`}/><YAxis dataKey="name" type="category" stroke="#8a9e8a" fontSize={11} width={70}/><Tooltip contentStyle={tt} formatter={v=>[`R$ ${v}`,'Lucro']}/><Bar dataKey="lucro" radius={[0,4,4,0]}>{byEsporte.map((e,i)=><Cell key={i} fill={e.lucro>=0?'#00e600':'#ff1744'}/>)}</Bar></BarChart></ResponsiveContainer>}</GlassCard>
        <GlassCard><div style={{fontWeight:700,fontSize:15,marginBottom:16}}>Performance por Casa</div>{byCasa.length===0?<div style={{color:'#8a9e8a',textAlign:'center',padding:30}}>Sem dados</div>:<div style={{display:'flex',flexDirection:'column',gap:10}}>{byCasa.map((c,i)=><div key={i} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',background:'#1a1e1a',borderRadius:10}}><div style={{width:8,height:8,borderRadius:'50%',background:CHART_COLORS[i%CHART_COLORS.length]}}/><div style={{flex:1,fontSize:13,fontWeight:600}}>{c.name}</div><div style={{fontSize:11,color:'#8a9e8a'}}>{c.total} ap.</div><div style={{fontSize:12,color:'#f5c518',width:40,textAlign:'right'}}>{c.winrate}%</div><div style={{fontSize:13,fontWeight:700,color:c.lucro>=0?'#00e600':'#ff1744',width:80,textAlign:'right'}}>R$ {c.lucro}</div></div>)}</div>}</GlassCard>
      </div>
    </div>
  )
}
...
function Bankroll({ bets, userId }) {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      ...
      <GlassCard><div style={{fontWeight:700,fontSize:15,marginBottom:20}}>Situacao do Bankroll</div><div style={{display:'flex',gap:14,flexWrap:'wrap'}}><StatCard label="Bankroll Inicial" value={`R$ ${Number(config.bankroll_inicial).toFixed(2)}`} color="#33dd33"/><StatCard label="Saldo Atual" value={`R$ ${stats.saldoAtual.toFixed(2)}`} color={stats.saldoAtual>=config.bankroll_inicial?'#00e600':'#ff1744'}/><StatCard label="Resultado Total" value={`R$ ${stats.lucroTotal.toFixed(2)}`} color={stats.lucroTotal>=0?'#00e600':'#ff1744'}/><StatCard label="Variacao" value={`${stats.pct}%`} color={parseFloat(stats.pct)>=0?'#00e600':'#ff1744'}/></div></GlassCard>
      {kelly&&<GlassCard glow="#f5c518"><div style={{fontWeight:700,fontSize:15,marginBottom:4}}>Criterio de Kelly</div><div style={{color:'#8a9e8a',fontSize:12,marginBottom:14}}>Baseado no seu historico</div><div style={{display:'flex',gap:24,flexWrap:'wrap'}}><div><div style={{fontSize:11,color:'#8a9e8a',marginBottom:4}}>% DO BANKROLL</div><div style={{fontSize:28,fontWeight:900,color:'#f5c518',fontFamily:"'Bebas Neue',cursive"}}>{kelly.kf}%</div></div><div><div style={{fontSize:11,color:'#8a9e8a',marginBottom:4}}>VALOR SUGERIDO</div><div style={{fontSize:28,fontWeight:900,color:'#f5c518',fontFamily:"'Bebas Neue',cursive"}}>R$ {kelly.val}</div></div></div><div style={{color:'#8a9e8a',fontSize:11,marginTop:10}}>Sugestao teorica. Nunca aposte mais do que pode perder.</div></GlassCard>}
      <GlassCard><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}><div style={{fontWeight:700,fontSize:15}}>Configuracoes</div>{!editing&&<button onClick={()=>setEditing(true)} style={{background:'#1e2a4a',border:'1px solid #00c80044',color:'#33dd33',borderRadius:8,padding:'6px 14px',cursor:'pointer',fontSize:12,fontWeight:700}}>Editar</button>}</div>{editing?(<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:14}}><div><label style={{fontSize:11,color:'#8a9e8a',fontWeight:700,display:'block',marginBottom:5}}>BANKROLL INICIAL (R$)</label><input type="number" value={form.bankroll_inicial} onChange={e=>setForm(f=>({...f,bankroll_inicial:e.target.value}))} style={inp}/></div><div><label style={{fontSize:11,color:'#8a9e8a',fontWeight:700,display:'block',marginBottom:5}}>STOP LOSS (%)</label><input type="number" value={form.stop_loss_percent} onChange={e=>setForm(f=>({...f,stop_loss_percent:e.target.value}))} style={inp} min="1" max="100"/></div><div style={{gridColumn:'1 / span 2',display:'flex',gap:10}}><button onClick={()=>setEditing(false)} style={{flex:1,background:'transparent',border:'1px solid #2d3a2d',color:'#8a9e8a',borderRadius:9,padding:10,cursor:'pointer'}}>Cancelar</button><button onClick={saveConfig} disabled={saving} style={{flex:2,background:'linear-gradient(135deg,#00c800,#007700)',border:'none',color:'#fff',borderRadius:9,padding:10,cursor:'pointer',fontWeight:700}}>{saving?'Salvando...':'Salvar'}</button></div></div>):(<div style={{display:'flex',gap:24}}><div><div style={{fontSize:11,color:'#8a9e8a'}}>BANKROLL INICIAL</div><div style={{fontSize:18,fontWeight:700,marginTop:3}}>R$ {Number(config.bankroll_inicial).toFixed(2)}</div></div><div><div style={{fontSize:11,color:'#8a9e8a'}}>STOP LOSS</div><div style={{fontSize:18,fontWeight:700,color:'#ff5252',marginTop:3}}>{config.stop_loss_percent}% = R$ {stats.stopLossVal.toFixed(2)}</div></div></div>)}</GlassCard>
    </div>
  )
}
...
function BetForm({ editData, userId, onSave, onClose }) {
  ...
  const lbl=(t)=><label style={{fontSize:11,color:'#8a9e8a',fontWeight:700,display:'block',marginBottom:5}}>{t}</label>
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'#000000cc',display:'flex',alignItems:'center',justifyContent:'center',zIndex:999,backdropFilter:'blur(6px)',padding:16}}>
      <div style={{background:'linear-gradient(135deg,#1e261e,#1e261e)',border:'1px solid #2d3a2d',borderRadius:20,padding:28,width:'100%',maxWidth:540,maxHeight:'92vh',overflowY:'auto',boxShadow:'0 20px 60px #00000066'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:18}}>
          <div style={{fontSize:17,fontWeight:800}}>{editData?.id?'Editar Aposta':'Nova Aposta'}</div>
          <button onClick={onClose} style={{background:'transparent',border:'none',color:'#8a9e8a',cursor:'pointer',fontSize:20,lineHeight:1}}>×</button>
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:13,marginTop:12}}>
          ...
          <div>{lbl('ODD')}<input type="number" step="0.01" min="1.01" value={form.odd} onChange={e=>upd('odd',e.target.value)} style={{...inp,color:'#f5c518',fontWeight:700,fontSize:16}}/></div>
        ...
        {safeNum(form.odd)>1&&safeNum(form.valor)>0&&(
          <div style={{marginTop:14,padding:'12px 16px',background:'#1e261e',border:'1px solid #00c80033',borderRadius:10,display:'flex',gap:20,alignItems:'center'}}>
            <div style={{flex:1}}>
              <div style={{fontSize:10,color:'#8a9e8a',fontWeight:700}}>RETORNO POTENCIAL</div>
              <div style={{fontSize:20,fontWeight:900,color:'#33dd33',fontFamily:"'Bebas Neue',cursive"}}>R$ {retornoPreview}</div>
            </div>
            <div style={{flex:1}}>
              <div style={{fontSize:10,color:'#8a9e8a',fontWeight:700}}>LUCRO POTENCIAL</div>
              <div style={{fontSize:20,fontWeight:900,color:safeNum(lucroPreview)>=0?'#00e600':'#ff5252',fontFamily:"'Bebas Neue',cursive"}}>R$ {lucroPreview}</div>
            </div>
          </div>
        )}
        ...
        <div style={{display:'flex',gap:10,marginTop:18}}>
          <button onClick={onClose} style={{flex:1,background:'transparent',border:'1px solid #2d3a2d',color:'#8a9e8a',borderRadius:10,padding:12,cursor:'pointer',fontWeight:600}}>Cancelar</button>
          <button onClick={handleSave} disabled={saving} style={{flex:2,background:saving?'#222822':'linear-gradient(135deg,#00c800,#007700)',border:'none',color:'#fff',borderRadius:10,padding:12,cursor:'pointer',fontWeight:700,fontSize:14,opacity:saving?0.7:1,transition:'all 0.2s'}}>{saving?'Salvando...':'Salvar Aposta'}</button>
        </div>
      </div>
    </div>
  )
    ...
    <>
      <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
        {[['todos',`Todos (${counts.todos})`],['pendente',`Pendentes (${counts.pendente})`],['ganhou',`Ganhos (${counts.ganhou})`],['perdeu',`Perdidos (${counts.perdeu})`]].map(([f,l])=>(
          <button key={f} onClick={()=>setFilter(f)} style={{background:filter===f?'#1e2a4a':'transparent',border:`1px solid ${filter===f?'#00c800':'#2d3a2d'}`,color:filter===f?'#fff':'#8a9e8a',borderRadius:8,padding:'6px 14px',fontSize:12,fontWeight:600,cursor:'pointer'}}>{l}</button>
        ))}
        <input placeholder="Buscar..." value={search} onChange={e=>setSearch(e.target.value)} style={{...inp,width:180,marginLeft:'auto'}}/>
        <button onClick={()=>exportCSV(filtered)} style={{background:'#1a2030',border:'1px solid #2d3a2d',color:'#8a9e8a',borderRadius:8,padding:'7px 14px',cursor:'pointer',fontSize:12,fontWeight:600,whiteSpace:'nowrap'}}>Exportar CSV</button>
        <button onClick={verificarResultados} disabled={verificando} style={{background:verificando?'#1a2030':'#1a2a1a',border:'1px solid #00e60044',color:'#00e600',borderRadius:8,padding:'7px 14px',cursor:'pointer',fontSize:12,fontWeight:700,whiteSpace:'nowrap',display:'flex',alignItems:'center',gap:5}}>
          {verificando ? '⏳ Verificando...' : '🔍 Verificar Resultados'}
        </button>
      </div>
      {verificMsg && <div style={{marginBottom:12,padding:'9px 14px',background:verificMsg.includes('atualizada')?'#00e60011':'#f5c51811',border:`1px solid ${verificMsg.includes('atualizada')?'#00e60033':'#f5c51833'}`,borderRadius:8,fontSize:12,color:verificMsg.includes('atualizada')?'#00e600':'#f5c518',fontWeight:600}}>{verificMsg}</div>}
      <div style={{background:'#222822',borderRadius:16,border:'1px solid #222822',overflowX:'auto'}}>
        <table style={{width:'100%',borderCollapse:'collapse',minWidth:820}}>
          <thead>
            <tr style={{background:'#1a1e1a',borderBottom:'1px solid #222822'}}>
              {[['data','Data'],['evento','Evento'],['esporte','Esporte'],['casa','Casa'],['odd','Odd'],['valor','Valor'],['status','Status'],['retorno','Retorno']].map(([f,l])=>(
                <th key={f} onClick={()=>hs(f)} style={{padding:'11px 14px',textAlign:'left',fontSize:10,fontWeight:700,color:'#8a9e8a',letterSpacing:1,textTransform:'uppercase',cursor:'pointer',whiteSpace:'nowrap',userSelect:'none'}}>{l} {sortField===f?(sortDir==='asc'?'^':'v'):''}</th>
              ))}
              <th style={{padding:'11px 14px',fontSize:10,color:'#8a9e8a',letterSpacing:1,textTransform:'uppercase'}}>Acoes</th>
            </tr>
          </thead>
          <tbody>
            {filtered.length===0&&<tr><td colSpan={9} style={{textAlign:'center',padding:40,color:'#8a9e8a'}}>Nenhuma aposta encontrada</td></tr>}
            {filtered.map((bet,i)=>(
              <tr key={bet.id} style={{borderBottom:'1px solid #1a2030',background:i%2===0?'transparent':'#0a0d18'}}>
                <td style={{padding:'12px 14px',fontSize:12,color:'#8a9e8a'}}>{bet.data}</td>
                <td style={{padding:'12px 14px',fontSize:13,fontWeight:600}}>
                  {bet.evento}
                  <div style={{fontSize:11,color:'#33dd33'}}>{bet.selecao}</div>
                  {bet.mercado&&<div style={{fontSize:10,color:'#8a9e8a'}}>{bet.mercado}</div>}
                  {bet.observacao?.includes('Resultado:')&&(
                    <div style={{fontSize:11,fontWeight:700,marginTop:3,color:bet.status==='ganhou'?'#00e600':'#ff5252',background:bet.status==='ganhou'?'#00e60011':'#ff174411',border:`1px solid ${bet.status==='ganhou'?'#00e60033':'#ff174433'}`,borderRadius:5,padding:'1px 7px',display:'inline-block'}}>
...
function BetApp({ user }) {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#1b1f1b',color:'#ffffff',fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{background:'linear-gradient(180deg,#1a1e1a,#1b1f1b)',borderBottom:'1px solid #222822',padding:'14px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:100,backdropFilter:'blur(10px)'}}>
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          <div style={{width:38,height:38,background:'linear-gradient(135deg,#00c800,#007700)',borderRadius:12,display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,fontWeight:900,color:'#fff',boxShadow:'0 4px 16px #00c80044'}}>B</div>
          <div><div style={{fontSize:17,fontWeight:800,letterSpacing:0.5}}>BetControl</div><div style={{fontSize:10,color:'#8a9e8a',letterSpacing:1}}>{user.email}</div></div>
        </div>
        <div style={{display:'flex',gap:8}}>
          <button onClick={()=>setShowForm(true)} style={{background:'linear-gradient(135deg,#00c800,#007700)',color:'#fff',border:'none',borderRadius:10,padding:'9px 18px',fontWeight:700,fontSize:13,cursor:'pointer',boxShadow:'0 4px 14px #00c80033'}}>+ Nova</button>
          <button onClick={()=>supabase.auth.signOut()} style={{background:'#1e261e',color:'#8a9e8a',border:'1px solid #2d3a2d',borderRadius:10,padding:'9px 14px',fontSize:13,cursor:'pointer'}}>Sair</button>
        </div>
      </div>

      <div style={{maxWidth:1100,margin:'0 auto',padding:'22px 16px 40px'}}>
        <div style={{display:'flex',gap:3,marginBottom:22,background:'#222822',borderRadius:14,padding:5,overflowX:'auto',flexWrap:'nowrap'}}>
          {TABS.map(([t,l])=>(
            <button key={t} onClick={()=>setTab(t)} style={{background:tab===t?'linear-gradient(135deg,#1e2a4a,#1a2540)':'transparent',border:`1px solid ${tab===t?'#00c80044':'transparent'}`,color:tab===t?'#fff':'#8a9e8a',borderRadius:10,padding:'8px 14px',fontSize:12,fontWeight:600,cursor:'pointer',whiteSpace:'nowrap',boxShadow:tab===t?'0 2px 12px #00c80022':'none',transition:'all 0.15s'}}>{l}</button>
          ))}
        </div>

        {loading?<div style={{textAlign:'center',padding:60,color:'#8a9e8a'}}>Carregando...</div>
...
export default function App() {
  ...
  if(session===undefined) return <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#1b1f1b'}}><div style={{color:'#8a9e8a'}}>Carregando...</div></div>
