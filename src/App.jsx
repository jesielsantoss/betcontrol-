...
function BetForm({ editData, userId, onSave, onClose }) {
  ...
  const F = ({k,label,type='text',full,opts}) => (
    <div style={{gridColumn:full?'1 / span 2':'auto'}}>
      ...
      {opts ? <select value={form[k]} onChange={e=>setForm(f=>({...f,[k]:e.target.value}))} style={inp}>{opts.map(o=><option key={o}>{o}</option>)}</select>
...
function Bankroll({ bets, userId }) {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      ...
      {kelly&&(<div style={{background:'#111724',border:'1px solid #ffab0033',borderRadius:16,padding:'20px 24px'}}><div style={{fontWeight:700,fontSize:15,marginBottom:4}}>🎯 Sugestão — Critério de Kelly</div><div style={{color:'#8892a4',fontSize:12,marginBottom:14}}>Baseado no seu histórico de apostas finalizadas</div><div style={{display:'flex',gap:24,flexWrap:'wrap'}}><div><div style={{fontSize:11,color:'#8892a4',marginBottom:4}}>% DO BANKROLL</div><div style={{fontSize:26,fontWeight:800,color:'#ffab00',fontFamily:"'Bebas Neue',cursive"}}>{kelly.kf}%</div></div><div><div style={{fontSize:11,color:'#8892a4',marginBottom:4}}>VALOR SUGERIDO</div><div style={{fontSize:26,fontWeight:800,color:'#ffab00',fontFamily:"'Bebas Neue',cursive"}}>R$ {kelly.val}</div></div></div><div style={{color:'#8892a4',fontSize:11,marginTop:10}}>⚠️ Sugestão teórica. Nunca aposte mais do que pode perder.</div></div>)}
      <div style={{background:'#111724',border:'1px solid #1e2538',borderRadius:16,padding:'20px 24px'}}>
